class User::DashboardController < User::BaseController
  ...
  def cart
    if params[:search_keyword].present?
      @book_details = current_user.book_details.book_search_by_keyword(params[:search_keyword])
    end
    ...
    if @book_details.present?
      ...
      @book_details = current_user.book_details.where('status = ?', true)
    end
  end

  def cart_destroy
    @book_detail = current_user.book_details.where(:id => params[:id]).first
    if @book_detail.update_attributes(:status => false)
      redirect_to cart_user_dashboard_index_path, 
